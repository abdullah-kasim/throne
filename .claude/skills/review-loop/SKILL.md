...
- `REVIEWER_MODEL` (with its harness) and `REVIEWER_EFFORT` — which model
  ...
  `"review-loop: missing REVIEWER_MODEL/REVIEWER_EFFORT"`.
...
**Not the route for campaign-scoped critique.** A todo bundle that needs a
scored, bounded review of its own modules uses `write-todos`' "Perceptual-quality
bundles" and `execute-todos`' "Rubric critique gates": critic and fixer are
both Shadows under the one campaign Alpha. This skill's reviewer/fixer-Alpha
model is for a standalone review-fix campaign against an already-shipped
target, and it is never invoked from inside a bundle to fix that bundle's own
slices — the Lord's ruling of 2026-09-02, after a multi-Alpha fix chain left
the Regent queueing corrections without end.
