...
human-steered, and the Lord switches them between opus and fable freely.
The gate still records what it observed in `runtime-model-evidence/
<phase>-exempt.json`, and `send-agent` says so in one stderr line, then
delivers. A Regent that sees that line has nothing to do; it is not a
blocker and is never reported as one. Campaign roles stay quarantined. See
`agent_docs/commands.md`, "The runtime-model gate and who it quarantines".
...
Three things this does NOT relax. The Stager still **commits its own work** —
working in main is not licence to leave a dirty tree, and everything else about
commit hygiene is unchanged. It still **never spawns an Alpha**: sharing
...
on the machine, and on 2026-08-24 one did exactly that — a `some-other-repo`
fix committed straight onto the Lord's own working branch in
`~/repos/some-other-repo`, no worktree, no campaign, no queue row. **Work in
...
3. **Cite relevant memory files by name** (agent-docs memories, known-trap
   ...
   of re-earning it.
3a. **A pull request as deliverable gets a human-named branch, filed as the
   target (Lord, 2026-09-08).** Create it in the target repo from the default
   branch first (`add/<feature>`, `fix/<bug>`, `update/<thing>` — the
   repository's own convention), then file `--target-branch` and
   `--pr-branch` both set to it. No PR branch, PR title, PR body or pushed
   commit message ever names `alpha`, `shadow`, an objective code, an agent,
   a bundle or a gate: the PR reads as the repository's contributor wrote it.
...
0. **Name the target repo.** A worktree is always created *for a specific repo* —
   ...
   `~/repos/some-other-repo`) the Lord points a campaign at. The command takes the
...
## Git identity — STOP and ask, never guess (Lord, 2026-09-08)

Every commit the court makes is signed with the Lord's identity for THAT
repository, taken from the `identity` section of the live throne
`config.user.ts` — a default `name`/`email` plus named `identities` chosen by
`remotes` rules on the origin (`github.com:ExampleCorp` and `github.example-corp.com`
can sign as one identity, `github.com:example-owner` as another). Every tab
is created with the identity for its cwd exported as `GIT_AUTHOR_*`/
`GIT_COMMITTER_*` (`src/herdr/herdr-tab.service.ts`), and the `bin/git` shim
re-resolves through `throne-cli git-identity --repo` whenever a commit targets
a repository with a different origin (`agent_docs/commands.md`). **Every
commit is GPG/SSH-signed** with the identity's `signingKey` — signing is
mandatory, an identity without a key is a STOP just like a missing one, and
the shim pins `commit.gpgsign=true` on the command line so no repository can
turn it off. Nothing sets a global or per-repository git identity. When that section is unset and the identity git would fall back to
is machine-local (`<user>@<hostname>`, `*.local`, `localhost`, no `@`), the
shim refuses with exit 66 and **STOP RIGHT THERE**. That refusal is one of the
few genuine stop-and-ask moments in the court, in the same class as a missing
`gh` login: do not guess an address, do not `git config --global` anything, do
not bypass the shim, do not commit with `--author`. Tell your supervisor; the
...
repo's own `agent_docs/MEMORY/`, a memory directive in its `AGENTS.md`/
`CLAUDE.md`, or a `memory-dir` executable on this machine, and only then the
throne-native `~/.throne/memories/<slug>`. Every mode keys on the repository's
main checkout, so every worktree of one project shares one directory: a
