...
export interface RepairRecipientDeps {
  ...
  readRegisteredSpawn(name: string): Promise<{ cwd: string; harness: string } | undefined>;
...
export async function resolveAgent(
  ...
  const matches = agents.filter(
    (agent) => sameAgentName(agent.name, name),
  );
  if (matches.length > 1) {
...
export async function repairRecipientByExactTabLabel(
  ...
  const tabs = (await deps.listTabs()).filter(
    (tab) =>
      tab.label === name ||
      resolvePersonaTabLabelToCanonical(tab.label, persistedSpawnLabel, deps.roleWordUnion) === name,
  );
...
export const DEFAULT_REPAIR_RECIPIENT_DEPS: RepairRecipientDeps = {
  ...
  // `resolvePersonaTabLabelToCanonical` already treats `''` as its own
  // "no persisted record" sentinel and declines to canonicalize -- a
  // field-absent or unresolved read both fall into that same declining
  // path, the same Found-else-'' collapse the pre-tristate read already
  // produced on any failure.
  readSpawnedTabLabel: async (name) => {
    const read = await readSpawnedTabLabel(name);
