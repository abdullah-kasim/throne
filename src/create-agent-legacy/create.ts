...
import { closeAgentTab } from "./legacy-herdr-tab.service.ts";
import {
  reconcileIndeterminateAgentStart,
  resumeRegisteredAgentInRestoredTab,
} from "../herdr/herdr-create.service.ts";
import { startAgent } from "./legacy-herdr-creation-orchestration.ts";
import { openMessageQueueStore } from "./legacy-message-queue.store.ts";
import { listAgents, resolveAgent } from "./legacy-herdr-runtime.service.ts";
import { sameAgentName } from "./legacy-herdr-identity-contracts.ts";
import { readUsageLogRaw } from "./legacy-usage-log.ts";
import type { CreateAgentDeps } from "./create.types.ts";
import { UsageReadersService } from "./legacy-usage-readers.service.ts";
import { realPlanUsageRemainingService } from "./legacy-plan-usage-remaining.service.ts";
...
import { readModelAllowlist } from "./model-allowlist.ts";
import {
  managedWorktreeRoot,
  treeNameFromPath,
  worktreesHome,
...
export async function runCreateAgent(
  ...
  if (registration.value.resuming && registration.value.emptyWorktree) {
    ...
    if (refusal !== undefined) {
      dependencies.writeStderr?.(
        `create-agent-legacy: refusing exact empty-workspace resume — ${refusal}. Nothing was registered or launched.\n`,
      );
      return 1;
    }
  }
  if (
    ...
    if (refusal !== undefined) {
      dependencies.writeStderr?.(
        `create-agent-legacy: refusing campaign Alpha launch — ${refusal}. Nothing was registered or launched.\n`,
      );
      return 1;
    }
  }
  ...
  if (policy.value.emptyWorktree) {
    ...
    if (refusal !== undefined) {
      dependencies.writeStderr?.(
        `create-agent-legacy: refusing ${refusal}. Nothing was registered or launched.\n`,
      );
      return 1;
    }
  }
...
export async function run(
  ...
  try {
    ...
    process.stderr.write(
      `create-agent-legacy: cannot resolve the authoritative live throne ledger: ${error instanceof Error ? error.message : String(error)}.\n`,
    );
    return 1;
  }
  ...
  if (!ledger.ok) {
    process.stderr.write(`create-agent-legacy: ${ledger.reason}.\n`);
    return 1;
  }
