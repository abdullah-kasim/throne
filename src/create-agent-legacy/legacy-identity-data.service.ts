...
import { containsSupervisionPollingLoop } from "../agentdata/identity-polling.ts";
...
// DELIBERATE DUPLICATION, not a cleanup target. Per AGENTS.md's migration
// law ("a replacement ships alongside what it replaces, not instead of it"):
// "The fallback shares nothing with what it survives: no store, schema,
...
// for." This legacy path therefore owns its own independent tristate read
// of `identity.md` rather than importing the corrected primitive from the
// non-legacy replacement's identity-data service -- importing it would make
// the legacy fallback reachable only while the replacement's read layer works,
// which is exactly when nobody needs the fallback. Consolidating this copy
// with the replacement later is a regression against that law, not a
// cleanup, however tempting the duplication looks.
//
...
const LegacyIdentityLineReadStatus = {
  Found: "found",
  FieldAbsent: "field-absent",
  ReadUnresolved: "read-unresolved",
} as const;
type LegacyIdentityLineReadStatus =
  (typeof LegacyIdentityLineReadStatus)[keyof typeof LegacyIdentityLineReadStatus];

type LegacyIdentityLineRead =
  | { status: (typeof LegacyIdentityLineReadStatus)["Found"]; value: string }
  | { status: (typeof LegacyIdentityLineReadStatus)["FieldAbsent"] }
  | {
      status: (typeof LegacyIdentityLineReadStatus)["ReadUnresolved"];
      error: unknown;
    };

async function readLegacyIdentityLine(
  name: string,
  prefix: string,
  baseDir: string,
): Promise<LegacyIdentityLineRead> {
  let body: string;
  try {
    body = await readFile(path.join(baseDir, name, "identity.md"), "utf8");
  } catch (error) {
    return { status: LegacyIdentityLineReadStatus.ReadUnresolved, error };
  }
  for (const line of body.split("\n")) {
    if (line.startsWith(prefix)) {
      return {
        status: LegacyIdentityLineReadStatus.Found,
        value: line.slice(prefix.length).trim(),
      };
    }
  }
  return { status: LegacyIdentityLineReadStatus.FieldAbsent };
}

function legacyIdentityLineValue(result: LegacyIdentityLineRead): string {
  switch (result.status) {
    case LegacyIdentityLineReadStatus.Found:
      return result.value;
    case LegacyIdentityLineReadStatus.FieldAbsent:
      return "";
    case LegacyIdentityLineReadStatus.ReadUnresolved:
      throw result.error instanceof Error
        ? result.error
        : new Error(
            `legacy identity line read unresolved: ${String(result.error)}`,
          );
  }
}
...
async function readIdentityLine(
  ...
  return legacyIdentityLineValue(
    await readLegacyIdentityLine(name, prefix, baseDir),
  );
}
