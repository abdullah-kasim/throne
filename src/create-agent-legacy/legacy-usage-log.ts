...
import type { CacheablePayload } from "../plan-usage-remaining/telemetry-core/cache.ts";
import type { UsageForecastSample } from "../plan-usage-remaining/telemetry-core/forecast.ts";
...
export function boundUsageLogRows(
  ...
  return rows
    ...
    .sort(
      (left, right) =>
        left.recordedAt - right.recordedAt || left.order - right.order,
    )
...
export function withInheritedFableWeeklyReset(
  ...
  return rows.map((row) => {
    ...
    return inheritedReset === undefined
      ? row
      : { ...row, reset_time: inheritedReset };
  });
}
...
async function closeWithoutChangingOutcome(
