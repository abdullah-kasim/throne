...
export async function writeQueueLaunchLinkage(
  ...
  try {
    ...
    if (!outcome.matched) {
      writeStdout(
        `create-agent-legacy: queue linkage not recorded for "${name}" — ${outcome.reason}.\n`,
      );
    }
  } catch (error) {
    writeStdout(
      `create-agent-legacy: queue linkage write-back failed for "${name}" (${error instanceof Error ? error.message : String(error)}) — launch proceeds unaffected.\n`,
    );
  } finally {
    store?.close();
  }
