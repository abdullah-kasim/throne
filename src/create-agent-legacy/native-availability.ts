...
export function nativeClaudeGateResult(opts: {
  ...
  if (availability.status === "exhausted") {
    ...
    if (opts.bypassZeroQuota === true) {
      return {
        kind: "proceed",
        message:
          `create-agent-legacy: OVERRIDE --bypass-zero-quota admitted final native Claude ${model} ` +
    ...
    return {
      kind: "refuse",
      message:
        `create-agent-legacy: refusing final native Claude ${model} launch target ` +
  ...
  return {
    kind: "warn",
    message:
      `create-agent-legacy: warning for final native Claude ${model} launch target ` +
  ...
  if (
    ...
    if (opts.bypassZeroQuota === true) {
      return {
        kind: "proceed",
        message:
          `create-agent-legacy: OVERRIDE --bypass-zero-quota admitted final Codex launch target ` +
    ...
    return {
      kind: "refuse",
      message:
        `create-agent-legacy: refusing final Codex launch target "${opts.name}" — fresh usage ` +
