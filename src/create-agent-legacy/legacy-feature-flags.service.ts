...
export function parseFeatureFlags(
  ...
  try {
    ...
    throw new Error(
      `Invalid throne feature flags in "${sourcePath}": expected JSON`,
      { cause },
    );
  }
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new Error(
      `Invalid throne feature flags in "${sourcePath}": expected an object`,
    );
  }
  ...
  if (
    ...
    typeof record[FEATURE_FLAG_NAMES.SEND_AGENT_FILE_BACKED_PAYLOADS] !==
      "boolean"
  ) {
    throw new Error(
  ...
  return {
    ...
    [FEATURE_FLAG_NAMES.SEND_AGENT_FILE_BACKED_PAYLOADS]:
      (record[FEATURE_FLAG_NAMES.SEND_AGENT_FILE_BACKED_PAYLOADS] as
        boolean | undefined) ??
...
export const REAL_FEATURE_FLAGS_SERVICE = new FeatureFlagsService(() =>
  loadFeatureFlags(),
);
