...
import { SpawnDataService } from "./legacy-spawn-data.service.ts";
...
export async function syncPersonaLedgerSymlinks(
  ...
  const expected = expectedPersonaSymlinkNames(
    activePersona,
    liveCanonicalDirs,
  );
  ...
  for (const [linkName, dirName] of expected) {
    await symlink(
      path.join(dataDir, dirName),
      path.join(dataDir, linkName),
      "dir",
    );
