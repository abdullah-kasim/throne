...
} from "../message-queue/delivery-failure-notice.store.ts";
...
} from "../message-queue/message-queue-work-item-state.ts";
export {
  InvalidWorkItemStateTransitionError,
  isForwardWorkItemStateTransition,
  MessageQueueWorkItemState,
...
import { createMessageQueueSchema } from "../message-queue/message-queue-schema.ts";
...
export class MessageQueueStore {
  ...
  insertWorkItem(item: NewWorkItem): WorkItemRow {
    ...
    const row = this.db
      .prepare(
        `INSERT INTO work_items (kind, payload, state, failure_reason, created_at, updated_at, terminal_at, dedupe_key, due_at)
         VALUES (?, ?, ?, NULL, ?, ?, NULL, ?, ?)
         RETURNING id, kind, payload, state, failure_reason, created_at, updated_at, terminal_at, due_at`,
      )
      .get(
        item.kind,
  ...
  readWorkItem(id: number): WorkItemRow | undefined {
    const row = this.db
      .prepare(
        `SELECT id, kind, payload, state, failure_reason, created_at, updated_at, terminal_at, due_at
  ...
  transitionWorkItemState(
    ...
    const failureReason =
      to === MessageQueueWorkItemState.Failed
  ...
  listWorkItemsByStates(
    states: readonly MessageQueueWorkItemState[],
  ): WorkItemRow[] {
  ...
  readLastRetentionSweep(): RetentionSweepResult | undefined {
    const row = this.db
      .prepare(
        `SELECT swept_count, swept_at FROM retention_sweep_runs WHERE id = 1`,
      )
      .get() as unknown as
      { swept_count: number; swept_at: number } | undefined;
    return row === undefined
      ? undefined
      : { sweptCount: row.swept_count, sweptAt: row.swept_at };
  }
