...
import { listTabs } from "./legacy-herdr-runtime.service.ts";
import { HerdrTabService } from "./legacy-herdr-tab.service.ts";
import { closeTab, createTab } from "./legacy-herdr-tab.service.ts";
import type {
...
export class CustomHarnessService {
  ...
  async run(
    ...
    try {
      ...
      writeStderr(
        `create-agent-legacy: custom harness one-shot launch failed for "${request.name}" ` +
