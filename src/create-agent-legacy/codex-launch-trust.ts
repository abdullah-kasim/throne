...
import { resolveTrustKey } from "./legacy-codex-trust.service.ts";
import {
...
export async function prepareCodexLaunchTrust(
  ...
  try {
    ...
    stderrWriter(deps)(
      `create-agent-legacy: refusing to spawn codex agent "${request.name}" — could not ` +
...
export async function verifyCodexLaunchTrust(
  ...
  if (agent === undefined) {
    writeStderr(
      `create-agent-legacy: spawned "${request.name}" but herdr did not register it within ` +
  ...
  try {
    ...
    writeStderr(
      `create-agent-legacy: codex "${request.name}" stopped at the folder-trust prompt, but ` +
  ...
  if (request.resuming) {
    writeStderr(
      `create-agent-legacy: codex "${request.name}" stopped at the folder-trust prompt and ` +
  ...
  writeStderr(
    `create-agent-legacy: codex "${request.name}" stopped at the folder-trust prompt for ` +
