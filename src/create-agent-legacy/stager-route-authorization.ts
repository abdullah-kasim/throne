...
function isAuthorizationEntry(
  value: unknown,
): value is StagerRouteAuthorizationEntry {
...
function isAuthorizationRegistry(
  value: unknown,
): value is StagerRouteAuthorizationRegistry {
  if (typeof value !== "object" || value === null) return false;
...
export async function readStagerRouteAuthorizationRegistry(
  ...
  const raw = await readFile(
    path.join(dataDir, "regent", AUTHORIZATION_FILE),
    "utf8",
  );
...
export function resolveStagerRouteAuthorization(opts: {
  ...
  if (opts.harness !== AUTHORIZED_HARNESS || opts.model !== AUTHORIZED_MODEL)
    return undefined;
