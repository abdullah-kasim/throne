...
export const TRUST_PROMPT_MARKER = 'Do you trust the contents of this directory?';
...
export function codexConfigPath(env: NodeJS.ProcessEnv = process.env): string {
  ...
  return path.join(home !== undefined && home !== '' ? home : path.join(os.homedir(), '.codex'), 'config.toml');
}

export function resolveTrustKey(cwd: string): string { return fs.realpathSync(cwd); }
...
function quote(value: string): string {
  return `"${[...value].map((ch) => {
    ...
    return code < 0x20 || code === 0x7f ? `\\u${code.toString(16).padStart(4, '0')}` : ch;
  }).join('')}"`;
}

function unquote(value: string): string {
  return value.replace(/\\(u[0-9a-f]{4}|U[0-9a-f]{8}|.)/gi, (_match, escape: string) => {
    const codes: Record<string, string> = { b: '\b', t: '\t', n: '\n', f: '\f', r: '\r', '"': '"', '\\': '\\' };
    return escape[0] === 'u' || escape[0] === 'U' ? String.fromCodePoint(Number.parseInt(escape.slice(1), 16)) : codes[escape] ?? escape;
  });
}
...
function headerKey(line: string): string | null {
  ...
  return match === null ? null : match[1] !== undefined ? unquote(match[1]) : match[2] ?? '';
}

export function isPathTrusted(configText: string, resolvedPath: string): boolean {
  let matching = false;
  for (const line of configText.split(/\r?\n/)) {
    const key = headerKey(line);
    if (key !== null) { matching = key === resolvedPath; continue; }
    if (/^\s*\[/.test(line)) { matching = false; continue; }
    if (matching && /^\s*trust_level\s*=\s*(?:"trusted"|'trusted')\s*(?:#.*)?$/.test(line)) return true;
  }
  return false;
}
...
export function appendTrustEntry(configText: string, resolvedPath: string): string {
...
export async function ensureCodexTrust(cwd: string, deps: Partial<EnsureCodexTrustDeps> = {}): Promise<void> {
  ...
  if (hasProjectBlock(text, key)) throw new Error(`codex config ${configPath} already has a [projects.${quote(key)}] entry whose trust_level is not "trusted". Refusing to append a duplicate table (duplicate tables are invalid TOML and would break codex entirely). Edit that entry to trust_level = "trusted" (or remove it) and retry.`);
  await (deps.writeConfig ?? writeConfig)(configPath, appendTrustEntry(text, key));
}
...
export async function probeCodexTrustPrompt(target: string, deps: Partial<ProbeCodexTrustPromptDeps> = {}): Promise<boolean> {
  ...
  for (;;) {
    try { if (isTrustPromptText(await read(target))) return true; } catch { /* keep polling through transient pane failures */ }
