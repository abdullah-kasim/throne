...
import { roleNameFor } from "../agentdata/identity-data.service.ts";
import {
...
const USAGE =
  "Usage: ./bin/throne-cli create-agent --model <model> " +
...
function validateOneShotEnvironment(
  ...
  for (const entry of flags.env ?? []) {
    ...
    if (separator < 1 || !/^[A-Za-z_][A-Za-z0-9_]*$/.test(name)) {
      writeStderr(
        "create-agent: each --env for --run-custom-harness-to-exit must be KEY=VALUE with a portable environment name; values are not printed. Nothing was launched.\n",
      );
      return false;
    }
    if (environmentNames.has(name)) {
      writeStderr(
        `create-agent: duplicate --env key "${name}" is refused for --run-custom-harness-to-exit; values are not printed. Nothing was launched.\n`,
      );
      return false;
    }
...
function validateOneShotFlags(
  ...
  if (flags.prompt !== undefined) {
    writeStderr(
      'create-agent: --prompt is resident composer input and is refused with --run-custom-harness-to-exit; put native prompt tokens after "--". Nothing was launched.\n',
    );
    return false;
  }
  if (flags["clear-environment"] !== true) {
    writeStderr(
      "create-agent: --run-custom-harness-to-exit requires --clear-environment. Nothing was launched.\n",
    );
    return false;
  }
  const pathFlags = [
  ...
  if (absent.length > 0) {
    writeStderr(
      `create-agent: --run-custom-harness-to-exit requires ${absent.map((key) => `--${key}`).join(", ")}. Nothing was launched.\n`,
    );
    return false;
  }
  if (
    ...
    writeStderr(
      "create-agent: --run-custom-harness-to-exit requires --timeout-ms as a positive integer. Nothing was launched.\n",
    );
    return false;
  }
  return true;
...
export async function prepareCreateAgentRequest(
  ...
  if (flags.harness !== undefined) {
    writeStderr(
      "create-agent: --harness is no longer caller-selectable; infer the harness from the canonical model registry by passing --model. Exact registered resumes retain their stored harness. Nothing was registered or launched.\n",
    );
    return { ok: false, code: 1 };
  }
  ...
  if (!oneShot) {
    ...
    if (supplied.length > 0) {
      writeStderr(
        `create-agent: ${supplied.map((key) => `--${key}`).join(", ")} require --run-custom-harness-to-exit. Nothing was registered or launched.\n`,
      );
      return { ok: false, code: 1 };
    }
  ...
  if (
    ...
    writeStderr(
      `create-agent: --effort must be an integer 1–6 (got "${flags.effort}")\n`,
    );
    return { ok: false, code: 1 };
  }
  ...
  if (passthrough !== undefined && requestedExecutable === undefined) {
    writeStderr(
      'create-agent: refusing a "--" passthrough argv without ' +
  ...
  if (requestedExecutable !== undefined) {
    ...
    if (pathRefusal !== undefined) {
      writeStderr(
        `create-agent: ${pathRefusal}. Nothing was registered or launched.\n`,
      );
      return { ok: false, code: 1 };
    }
  }
  ...
  if (casingGate.refuse) {
    writeStderr(
      `create-agent: ${casingGate.reason} Nothing was registered or launched.\n`,
    );
    return { ok: false, code: 1 };
  }
  ...
  if (presetGate.refuse) {
    writeStderr(`create-agent: ${presetGate.reason}\n`);
    return { ok: false, code: 1 };
  }
  const crossPrefix = crossRolePrefixGuard({ role, name: requestedName });
  if (crossPrefix.refuse) {
    writeStderr(
      `create-agent: refusing --role ${role} --name "${requestedName}" — the name ` +
  ...
  if (
    ...
    writeStderr(
      "create-agent: --empty-worktree is reserved for campaign Alpha or Shadow launches. Nothing was registered or launched.\n",
    );
    return { ok: false, code: 1 };
  }
  if (
    ...
    writeStderr(
      "create-agent: --empty-worktree cannot use a managed Git worktree --cwd; use the generated empty workspace path or omit --cwd. Nothing was registered or launched.\n",
    );
    return { ok: false, code: 1 };
  }
  ...
  if (mismatch !== undefined) {
    writeStderr(
      `create-agent: refusing to spawn "${name}" inside git tree ` +
  ...
  if (flags["deliverable-shape"] !== undefined) {
    if (flags["deliverable-shape"] !== "verdict-only") {
      writeStderr(
        `create-agent: --deliverable-shape "${flags["deliverable-shape"]}" is refused — ` +
    ...
    if (isTerminalAbsorbOrDeliveryShadowName(canonicalName)) {
      writeStderr(
        `create-agent: --deliverable-shape verdict-only is refused for "${canonicalName}" — ` +
