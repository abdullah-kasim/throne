...
export async function resolveRegistration(
  ...
  if (!nameTaken) {
    ...
    if (nameRefusal !== undefined) {
      writeStderr(
        `create-agent: refusing fresh agent — ${nameRefusal}. Nothing was registered or launched.\n`,
      );
  ...
  if (!(await deps.registrationExists(request.name))) {
    writeStderr(
      `create-agent: an agent named "${request.name}" already exists — names must be ` +
  ...
  if (runningAgent !== undefined) {
    writeStderr(
      `create-agent: registered agent "${request.name}" is already running — refusing ` +
  ...
  if (stored === undefined) {
    writeStderr(
      `create-agent: registered agent "${request.name}" has a missing, corrupt, or ` +
  ...
  if (conflicts.length > 0) {
    writeStderr(
      `create-agent: refusing to resume registered agent "${request.name}" because ` +
