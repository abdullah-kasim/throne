import { buildCustomLaunchArgv, buildLaunchArgv } from "../harness-routing/harness.ts";
import {
    ...
    if (!request.resuming && error instanceof AgentStartIndeterminateError) {
      try {
        ...
        if (result.kind === "already-live") {
          ...
          writeStderr(
            `create-agent: launch of "${request.name}" timed out after Herdr issued it, ` +
      ...
      writeStderr(
        `create-agent: registered agent "${request.name}" could not be proven live or ` +
...
export async function runResidentAgent(
  ...
  const delivery = await deliverAgentOpeningPrompt(
    request,
    deps,
    prompts.delivered,
    start.value,
