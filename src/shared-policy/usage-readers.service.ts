...
export class UsageReadersService {
  ...
  constructor(
    adaptersOrDependencies: UsageAdaptersService | UsageReaderDependencies | PlanUsageRemainingService =
      new UsageAdaptersService(),
    dependencies?: UsageReaderDependencies,
    planUsage?: PlanUsageRemainingService,
  ) {
    const directPlanUsage = adaptersOrDependencies instanceof PlanUsageRemainingService
  ...
  return {
    getClaudeUsage: () => {
      if (planUsage === undefined) {
        throw new Error('PlanUsageRemainingService is required for Claude usage');
      }
